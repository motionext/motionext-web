  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      {legalDocuments.map((doc) => (
        <a key={doc.title} href={doc.href} className="group no-underline">
          <Card className="h-full transition-all duration-300 hover:shadow-lg hover:-translate-y-1">
            <CardHeader>
              ...
              <CardTitle className="text-xl mb-2 no-underline">{doc.title}</CardTitle>
              <CardDescription className="no-underline">{doc.description}</CardDescription>
            </CardHeader>
          </Card>
        </a>
      ))}
